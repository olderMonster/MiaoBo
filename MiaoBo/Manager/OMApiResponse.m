...
@property (nonatomic , assign , readwrite)PDApiResponseStatus status;
...
- (instancetype)initWithResponseString:(NSString *)responseString requestId:(NSNumber *)requestId request:(NSURLRequest *)request
                          responseData:(NSData *)responseData status:(PDApiResponseStatus)status{
    self = [super init];
    if (self) {
        self.status = status;
...
- (PDApiResponseStatus)responseStatusWithError:(NSError *)error
{
    if (error) {
        PDApiResponseStatus result = PDApiResponseStatusErrorNoNetwork;
        
        // 除了超时以外，所有错误都当成是无网络
        if (error.code == NSURLErrorTimedOut) {
            result = PDApiResponseStatusErrorNoNetwork;
        }
        return result;
    } else {
        return PDApiResponseStatusSuccess;
    }
}
